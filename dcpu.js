...
decodeValue: function(index, offset, val, labels, logger) {
  ...
  var value = Assembler.parseExpression(index, offset, val, 0, labels, logger);
  if (!value) {
    return false;
  }

  value = Assembler.simplifyExpression(value);
...
compileLine: function(index, offset, line, labels, logger) {
    ...
    for (var j = 0; j < vals.length; j++) {
      if (vals[j].length > 0) {
        if (vals[j].charAt(0) == '"') {
          ...
          // FIXME turn these 2 calls into a single function. they happen together all over the place.
          var value = Assembler.parseExpression(index, offset, vals[j], 0, labels, logger);
          if (value) {
            value = Assembler.simplifyExpression(value);
          }
          if (!value) {
  ...
  if (info.op == "ORG") {
    ...
    var value = Assembler.parseExpression(index, offset, vals[0], 0, labels, logger);
    if (value) {
      value = Assembler.simplifyExpression(value);
    }
    if (!value) {
  ...
  if (i >= 0) {
    ...
    if (i == -1) {
      if (vals.length == 1 && vals[0][0] == "=") {
        var value = Assembler.parseExpression(index, offset, vals[0].substr(1), 0, labels, logger);
        if (value) {
          value = Assembler.simplifyExpression(value);
        }
        if (!value) {
